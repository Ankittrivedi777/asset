...
static uint32_t ble_char_accel_add(ble_mpu_t * p_mpu)
{
    ...
    attr_char_value.max_len     = sizeof(sensor_values_t);
    attr_char_value.init_len    = sizeof(sensor_values_t);
    uint8_t value[6]            = {0};
...
// ALREADY_DONE_FOR_YOU: Function to be called when updating characteristic value
void ble_mpu_update_sensor(ble_mpu_t *p_mpu, sensor_values_t * sensor_values)
{
    // Send value if connected and notifying
    if (p_mpu->conn_handle != BLE_CONN_HANDLE_INVALID)
    {
        uint16_t               len = sizeof(sensor_values_t);
        ble_gatts_hvx_params_t hvx_params;
        memset(&hvx_params, 0, sizeof(hvx_params));
        ...
        hvx_params.offset = 0;
        hvx_params.p_len  = &len;
        hvx_params.p_data = (uint8_t*)sensor_values;  
